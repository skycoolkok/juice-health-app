...
export default function RecipesClient({ cardHint }: RecipesClientProps) {
  ...
  const effectiveCardHint = cardHint ?? tRecipes('cardHint');
      ...
      <section className="space-y-6 rounded-3xl bg-white p-6 shadow-sm sm:p-10">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            {hasActiveFilters ? (
              <p className="text-xs text-slate-500">{effectiveCardHint}</p>
            ) : (
        ...
        {error ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {recipes.map((recipe) => (
              <RecipeCard key={recipe.id} recipe={recipe} locale={locale} footerHint={effectiveCardHint} />
            ))}
          </div>
        )}
      </section>
